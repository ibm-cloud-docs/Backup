...
9. To exclude databases from the backup job, do one or more of the following in the Select Databases for Backup box:
   - To exclude specific databases from the backup job, select the check box for each database, and then click Exclude. The excluded databases appear in the Backup Set box with a minus sign.
