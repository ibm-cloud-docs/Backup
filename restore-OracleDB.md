...
8. Select one of the File Overwrite options. This option specifies how to restore a file to a location where a file with the same name exists.
   ...
   - To add a numeric extension (for example, .0001) to a restored file name, select **Do not overwrite existing files**. For example, if you restore a file named “filename.txt” to a location where a file with the same name is already present, an extension is added to the restored file name (“filename.txt.0001”).
